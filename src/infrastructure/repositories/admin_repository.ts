...
class AdminRepository implements IAdminRepository {
    ...
    async findAllUsers(page: number, limit: number): Promise<{users:IUser[],total:number}> {
        const users_list = await users.find().skip((page-1)*limit).limit(limit)
    ...
    async findAllServiceProviders(page: number, limit: number): Promise<{sp:IService_provider[],total:number}> {
        const sp_list = await service_provider.find().skip((page-1)*limit).limit(limit)
